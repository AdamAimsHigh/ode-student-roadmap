...
Theme Toggle: Here is the architectural implementation for the theme toggle:
1. CSS Variable Definitions
In your global stylesheet, define the theme variables. By using custom properties, you can switch the entire site appearance by changing one attribute on the root element.
CSS
:root {
  --bg-color: #ffffff;
  --text-color: #1a1a1a;
...
2. State Management and Toggle Logic
Add this function to your JavaScript orchestrator. It handles the UI switch and updates the local storage, ensuring the user choice persists across sessions.
JavaScript
...
3. UI Integration
You should place the toggle button inside the Global Header defined in your Master Architecture Specification. Since you are focusing on a clean, professional layout, a simple button or icon switch works well.
Button Location: Top right corner of the Global Header.
Labeling: Use clear text labels like "Dark Mode" and "Light Mode" or descriptive icons, ensuring you do not use prohibited symbols in the button text.



Section 3: Curriculum Data Structure
This is where token-maxing really happens. Instead of making the AI guess how to organize the site, we feed it a clean data schema. This schema ensures the model builds the exact learning path required without wasting tokens guessing how to organize the site.
To build this section properly, we need to map the videos into a structured JSON hierarchy (e.g., Module -> Chapter -> Video URL -> Desmos Widget ID).
...
The canonical curriculum JSON is defined once in Section 3 of this document and mirrored in app/data/curriculum.json and app/js/curriculum-data.js. It is not duplicated here to prevent divergence.
</curriculum-data-structure>
