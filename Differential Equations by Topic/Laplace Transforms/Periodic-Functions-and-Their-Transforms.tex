...
The key move is a \emph{time-shift substitution}. Consider the general $n$-th term, integrated over the interval $[nP, (n+1)P]$. We substitute $t = \tau + nP$, so that $d t = d\tau$. As $t$ runs across $[nP, (n+1)P]$, the new variable $\tau$ runs across $[0, P]$, collapsing every interval back onto the very first period:
\[ \int_{nP}^{(n+1)P} e^{-st}f(t)\,dt = \int_{0}^{P} e^{-s(\tau + nP)}f(\tau + nP)\,d\tau \]
...
\[ e^{-s(\tau + nP)} = e^{-s\tau}\cdot e^{-nPs} \]
The factor $e^{-nPs}$ is constant with respect to $\tau$, so it pulls straight out of the integral:
\[ \int_{nP}^{(n+1)P} e^{-st}f(t)\,dt = e^{-nPs}\int_{0}^{P} e^{-s\tau}f(\tau)\,d\tau \]
Summing this identity over every cycle $n = 0, 1, 2, \dots$ factors the shared single-period integral out front, leaving behind a sum of the constants $e^{-nPs}$:
...
\[ \color{workpurple} F(s) = \frac{1}{1 - e^{-2s}} \int_0^2 e^{-st} t \,dt \]
We use Integration by Parts for $\int_0^2 t e^{-st} \,dt$ with the choices \textcolor{workpurple}{$u = t$}, \textcolor{workpurple}{$du = dt$}, \textcolor{workpurple}{$dv = e^{-st}\,dt$}, \textcolor{workpurple}{$v = -\frac{1}{s}e^{-st}$}:
\[ \color{workpurple}
\int_0^2 t e^{-st} \,dt = \left[ -\frac{t}{s}e^{-st} \right]_0^2 - \int_0^2 \left( -\frac{1}{s}e^{-st} \right) \,dt
\]
\[ \color{workpurple}
= \left( -\frac{2}{s}e^{-2s} - 0 \right) - \left[ \frac{1}{s^2}e^{-st} \right]_0^2
\]
\[ \color{workpurple}
= -\frac{2}{s}e^{-2s} - \left( \frac{1}{s^2}e^{-2s} - \frac{1}{s^2} \right) = \frac{1}{s^2} - \frac{1}{s^2}e^{-2s} - \frac{2}{s}e^{-2s}
\]
Factoring out the terms and combining with the multiplier:
\[ \color{workpurple}
F(s) = \frac{1}{1 - e^{-2s}} \left( \frac{1 - e^{-2s} - 2se^{-2s}}{s^2} \right)
\]
...
\[ \color{workpurple} F(s) = \frac{1}{1 - e^{-\pi s}} \int_0^\pi e^{-st} \sin(t) \,dt \]
The standard integral identity \textcolor{workpurple}{$\int e^{at} \sin(bt) \,dt = \frac{e^{at}}{a^2+b^2}(a \sin(bt) - b \cos(bt))$} gives us:
\[ \color{workpurple}
\int_0^\pi e^{-st} \sin(t) \,dt = \left[ \frac{e^{-st}}{s^2 + 1} (-s \sin(t) - \cos(t)) \right]_0^\pi
\]
Evaluating at the bounds (\textcolor{workpurple}{$\sin(\pi)=0$}, \textcolor{workpurple}{$\cos(\pi)=-1$} and \textcolor{workpurple}{$\sin(0)=0$}, \textcolor{workpurple}{$\cos(0)=1$}):
\[ \color{workpurple}
...
\[ \color{workpurple} F(s) = \frac{1}{1 - e^{-3s}} \int_0^3 e^{-st} (u_1(t) - u_2(t)) \,dt \]
