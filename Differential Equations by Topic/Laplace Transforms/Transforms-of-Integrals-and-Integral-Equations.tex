...
\smallskip
\noindent\textit{Cross-check by partial fractions.} Writing $\dfrac{1}{s(s^2+4)} = \dfrac{A}{s} + \dfrac{Bs + C}{s^2+4}$ and clearing gives $1 = A(s^2+4) + (Bs+C)s$. At $s = 0$: $1 = 4A \Rightarrow A = \tfrac14$. Matching $s^2$: $0 = A + B \Rightarrow B = -\tfrac14$. Matching $s^1$: $0 = C$. So $\dfrac{1/4}{s} - \dfrac{1}{4}\cdot\dfrac{s}{s^2+4}$ inverts to $\tfrac14 - \tfrac14\cos 2t$, the same result.
...
\smallskip
\noindent\textit{Cross-check by full partial fractions.} $\dfrac{1}{s^2(s+1)} = \dfrac{A}{s} + \dfrac{B}{s^2} + \dfrac{C}{s+1}$ gives $1 = A s(s+1) + B(s+1) + C s^2$. At $s=0$: $B = 1$. At $s=-1$: $C = 1$. Matching $s^2$: $0 = A + C \Rightarrow A = -1$. So $-\dfrac1s + \dfrac{1}{s^2} + \dfrac{1}{s+1}$ inverts to $-1 + t + e^{-t}$, matching.
...
\bigskip
\noindent\textbf{Solution 8: $\displaystyle x'(t) + \int_0^t x(\tau)\,d\tau = 1, \quad x(0) = 0$.}\\[2pt]
\textbf{Step 1 --- Transform each term.} The derivative rule gives $\mathcal{L}\{x'\} = sX(s) - x(0) = sX(s)$ since $x(0) = 0$. The integral rule gives $\int_0^t x \to \dfrac{X(s)}{s}$, and $\mathcal{L}\{1\} = \dfrac1s$:
\[ \color{workpurple} sX(s) + \frac{X(s)}{s} = \frac{1}{s}. \]

\textbf{Step 2 --- Factor out $X(s)$.}
...
\smallskip
\noindent\textit{Check.} $x'(t) = \cos t$ and $\int_0^t \sin\tau\,d\tau = 1 - \cos t$, so $\cos t + (1 - \cos t) = 1$. \checkmark The initial condition $x(0) = \sin 0 = 0$ also holds.
